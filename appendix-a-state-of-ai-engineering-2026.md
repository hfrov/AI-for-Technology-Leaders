# Appendix A - State of AI Engineering 2026

*Background research note: the 2026 data set behind this book's core argument, plus the stakeholder-action table referenced in Chapter 2. This is reference material, not a chapter to read start-to-finish - come back to it when you need a citation or a number for your own business case.*
...
**Review, not generation, is the new bottleneck.** Output per engineer is up roughly 60% year over year, but a reviewer can still only meaningfully process the same number of pull requests they always could - Faros AI's 2025 telemetry across 10,000+ developers found teams with high AI adoption merged 98% more pull requests, while PR review time rose 91%, and org-level DORA metrics showed no measurable improvement despite the individual-level gains. LinearB's 2026 benchmarks put AI-generated PRs waiting 4.6x longer before a reviewer even picks them up. The cognitive load has changed shape, not just volume: reviewers are no longer mainly checking correctness, they're judging necessity - does this abstraction earn its weight, is this edge case worth the complexity - which takes more effort per PR, not less, at the exact moment PR volume is exploding. Senior engineers report spending 20-35% more time on review when juniors lean heavily on AI assistants, and one 2025 study found reviewers spend 4.3 minutes on an AI-generated suggestion versus 1.2 minutes on human-written code, with 11.8% more review rounds. Leadership implication: any ROI case built on "AI writes code N times faster" that doesn't also model reviewer capacity is measuring the wrong bottleneck - throughput at the generation step means nothing if it just queues up at the review step instead.
...
| **CHRO / People leaders** | Skills demand is shifting from code-writing to systems thinking and AI orchestration; 85% of leaders say adaptability is critical, only 7% believe they're actually leading on it. | Redesign career ladders around orchestration and judgment, not typing throughput. Treat reskilling as a scheduled capability investment (my Domain 3), not a reactive layoff-adjacent scramble. |
| **Engineering managers** | 70% of leaders report quality has already degraded because code generation outpaced test capacity. Review time is up 91% while merges are up 98% (Faros AI) - review, not generation, is now the constraint. | Track AI-attributed regression rate and review-confidence score as first-class engineering metrics (Domain 7 Outcomes), the same way security incidents are tracked. Staff reviewer capacity as a planned line item, not an assumed free resource, when sizing AI rollout. |
...
| Teams with high AI adoption merged 98% more PRs, but PR review time rose 91%; org-level DORA metrics showed no measurable gain | Faros AI 2025 telemetry study, 10,000+ developers, 1,255 teams |
| AI-generated PRs wait 4.6x longer before a reviewer picks them up | LinearB 2026 benchmarks |
| Senior engineers spend 20-35% more time on review when juniors lean heavily on AI assistants | 2026 engineering leadership surveys |
| Reviewers spend 4.3 min on an AI-generated suggestion vs. 1.2 min on human-written code; 11.8% more review rounds | 2025 developer review-time study |
...
Some figures above are hard counts from named studies (Gartner, DORA, Veracode); others are directional estimates from industry surveys with smaller sample sizes. Where a number matters for a board-level business case, verify it against the primary source in the list below before citing it externally - treat everything here as a well-sourced starting point, not a final audited figure.
