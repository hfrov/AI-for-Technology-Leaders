...
## Team Model
It can be tempting to put the most passionate employees to this type of project, they should be, but at the same time make sure the group also includes people who are passionate about quality, the business, the domain, data and ML. You will need people with a high degree of knowledge on how to set this up in practical terms, e.g. how does team members get access, use and report a problem? make sure you have those onboard who care about responsible AI practices. My recommendation is to start small, focus on the end result in mind, whether that is to scale up engineering, make customers successfull, enhance productivity, scale product innovation and so on. In my experience bringing the business objectives front and center gives a common understanding of why across engineering, product, c-suite, board and other key stakeholders. once the objectives are clear, begin with the economy, what kind of budget is needed, then understand the first objective, can we manually make a model do a simple test based on our code base? can it reach all the code and make sensible tests. gradually build test by test case (not for the code but for whatever the team has in objectives), monitor financials, what was the cost to reach these test results? now comes the part where the backlog should be transformed into actual tasks and the team can continue to build its scope. 
